...
class RNN_mLor_2D(nn.Module):
  ...
  def param(self):
    ...
    self.C1[MX_start:MX_end, MY_start:MY_end] = eps_inf / dt / (eps_inf / dt + temp1 * 0.5)
    self.C2[MX_start:MX_end, MY_start:MY_end] = 0.5 / (eps_inf / dt + temp1 * 0.5)
  ...
  def forward(self,src):
    ...
    for t in range(NSTEPS):
        src_val = E0 * math.exp(-(dt * t - n_T * T) * (dt * t - n_T * T) / (T * T)) ## Pulse_idx = 1
        src[CX, CY] = src_val
...
plt.savefig("outputs/inhomo_tio2/obs_Ez@center.png", dpi=300)
plt.show()